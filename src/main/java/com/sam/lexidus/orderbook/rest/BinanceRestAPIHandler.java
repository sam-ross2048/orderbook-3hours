...
public class BinanceRestAPIHandler {
    ...
    private static final String BINANCE_DEPTH_API_URL = "https://api.binance.com/api/v3/depth?symbol=BNBBTC&limit=1000";
    ...
    private static void handleSnapshot(ResponseEntity<String> response, OrderBook orderBook) {
        try {
            ...
            for(int i=0; i<bidArray.size();i++) {
                Float price = Float.valueOf(bidArray.get(i).get(0).textValue());
                Float size = Float.valueOf(bidArray.get(i).get(1).textValue());
            ...
            for(int i=0; i<askArray.size();i++) {
                Float price = Float.valueOf(askArray.get(i).get(0).textValue());
                Float size = Float.valueOf(askArray.get(i).get(1).textValue());
                Order order = new Order(size, price);
                asks.add(order);
            }
